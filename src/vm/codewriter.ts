export default class CodeWriter {
  ...
  public writeLabel(label: string): void {
    this.write([`(${this.currentFunction}$${label}) // label ${label}`]);
  }

  public writeGoto(label: string): void {
    this.write([`@${this.currentFunction}$${label} // goto ${label}`, "0;JMP"]);
  }
  ...
  public writeReturn(): void {
    this.write(["@DO_RETURN // return", "0;JMP"]);
  }
  ...
  public writeFunction(functionName: string, numLocals: number): void {
    ...
    const out = [`(${functionName}) // function ${functionName} ${numLocals}`];
    if (numLocals) {
      if (numLocals === 1) {
        this.write(out);
  ...
  public writePush(segment: string, index: number): void {
    ...
    switch (segment) {
      ...
      case "pointer":
      case "temp":
        out = [`@R${index + CodeWriter.segmentMemoryMap[segment]}`, "D=M"];
        break;
      case "static":
        out = [`@${this.fileName}.${index}`, "D=M"];
        break;
      case "local":
      case "argument":
      case "this":
      case "that":
        if (index === 0) {
          out = [CodeWriter.segmentMemoryMap[segment], "A=M", "D=M"];
        } else if (index === 1) {
          out = [CodeWriter.segmentMemoryMap[segment], "A=M+1", "D=M"];
        } else if (index === 2) {
          out = [CodeWriter.segmentMemoryMap[segment], "A=M+1", "A=A+1", "D=M"];
        } else {
          out = [
