...
export default class Dot {
    ...
    if (eq(this.#brain.getStep(), Dot.#minStep)) {
      ...
      if (
        lt(this.#posX, 2) |
          ...
          gt(this.#posX, 510) |
          gt(this.#posY, 254) ||
        eq(Dot.#obstacles[Main.getGridIndex(this.#posX, this.#posY)], neg(1))
      ) {
        this.#dead = neg(1);
      } else {
        if (
          not(
            gt(abs(sub(this.#posX, Dot.#goalX)), 3) |
              gt(abs(sub(this.#posY, Dot.#goalY)), 3)
          )
        ) {
  ...
  calculateFitness() {
    if (this.#reachedGoal) {
      return max(
        10000,
        sub(32767, mult(Dot.#stepWeight, this.#brain.getStep()))
      );
    }
